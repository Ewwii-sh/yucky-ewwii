...
pub fn report_diag_error(source: &str, path: &str, error: &DiagError) {
    ...
    let mut builder = Report::build(ReportKind::Error, (path, 0..0)).with_message(&diag.message);

    for label in &diag.labels {
        let span = label.range.start..label.range.end;
        let msg = label.message.clone();
        builder =
            builder.with_label(Label::new((path, span)).with_message(msg).with_color(Color::Red));
    }
    ...
    builder.finish().print((path, Source::from(source))).unwrap();
}
